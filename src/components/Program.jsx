        ...
        <>
            <div className="bg-neutral-900 px-10 pt-10 pb-6 m-10 rounded-xl shadow-xs border border-neutral-700 shadow-neutral-800 lg:w-1/2 lg:mx-auto">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <select onChange={e => setSelectedProgram(Program.find(program => program.program_id === Number(e.target.value)))} disabled={isOnlyOneProgram} className="select select-lg w-full bg-neutral-800 p-3 rounded-xl border-neutral-700 text-neutral-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-green-600">
                        {
                            Program.map((program, index) => (
                                <option key={program.program_id} value={program.program_id} className="bg-neutral-800 hover:bg-green-600">{program.name}</option>
                            ))
                        }
                    </select>
                    <div className=" flex justify-center gap-4 mt-4 lg:mt-0 lg:ml-4 lg:justify-start">
                        <EditProgram hoverEdit={hoverEdit} setHoverEdit={setHoverEdit} programId={selectedProgram.program_id} />
                        <DeleteProgram isOnlyOneProgram={isOnlyOneProgram} hoverTrash={hoverTrash} setHoverTrash={setHoverTrash} programId={selectedProgram.program_id} />
            ...
            <ProgramView programId={selectedProgram.program_id} />
        </>
    )
}